...
int main(){
    ...
    switch(mode) {
        case 'w':			//register only to read
            printf("please enter your account and keycard\n");
            scanf("%d", &studentID);
        ...
        case 'r': 			//sigh in only to write
            ...
            scanf("%s %d %d %d %s %s", &name, &classroom, &age, &studentID, &key, &account);   //输入数据
